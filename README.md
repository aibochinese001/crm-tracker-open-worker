...
如果你是 AI 编码/部署 Agent（或想一键自动化部署），请直接阅读 **[AGENTS.md](AGENTS.md)**：
包含确定性部署命令、Token 权限清单、Secrets 清单、常见报错对照表，照单执行即可，无需猜测。
...
```bash
git clone https://github.com/aibochinese001/crm-tracker-open-worker.git
cd crm-tracker-open-worker
npm install
...
1. `npm run setup` — 自动创建 / 复用 KV（`SUBSCRIPTIONS_KV`）与 D1（`crm-saas-db`），抓取真实 ID 回填 `wrangler.toml`，执行 `schema.sql` 初始化表结构，并生成 `SMTP_BRIDGE_SECRET` 到 `.dev.vars`
