...
async function handleUpdateConfig(request, env) {
  try {
    ...
    const updatedConfig = {
      ...
      /* ---- SaaS 配置 ---- */
      // 免费用户配额（0 = 禁止免费用户创建）
      FREE_QUOTA: Number.isFinite(Number(newConfig.FREE_QUOTA))
        ? Math.max(0, Math.floor(Number(newConfig.FREE_QUOTA)))
        : (config.FREE_QUOTA != null ? config.FREE_QUOTA : 3),
      // 注册开关
      ALLOW_REGISTRATION: newConfig.ALLOW_REGISTRATION !== false && newConfig.ALLOW_REGISTRATION !== '0',
      // 易支付网关
      EPAY_API_URL: (newConfig.EPAY_API_URL || config.EPAY_API_URL || '').trim(),
      EPAY_PID: (newConfig.EPAY_PID || config.EPAY_PID || '').trim(),
      EPAY_KEY: mergeSecretField(config, newConfig, 'EPAY_KEY', clearSecretFields),
      // 开放的支付方式（未列出的对用户隐藏）；旧值 stripe 自动映射为 fiatstripe
      PAY_CHANNELS: (() => {
        const VALID = ['alipay', 'wxpay', 'usdt', 'fiatstripe', 'ecny'];
        let list;
        if (Array.isArray(newConfig.PAY_CHANNELS) && newConfig.PAY_CHANNELS.length > 0) {
          list = newConfig.PAY_CHANNELS
            .map((c) => (c === 'stripe' ? 'fiatstripe' : c))
            .filter((c) => VALID.includes(c));
        } else if (Array.isArray(newConfig.PAY_CHANNELS)) {
          list = ['alipay', 'wxpay'];
        } else {
          list = (config.PAY_CHANNELS || ['alipay', 'wxpay'])
            .map((c) => (c === 'stripe' ? 'fiatstripe' : c));
        }
        return list;
      })()
