 ...
 * @property {string} [subId] 关联的客户 ID（写日志用）
...
export async function dispatch(payload, config, options = {}) {
  ...
  for (const r of results) {
    ...
    if (options.env && options.subId) {
      try {
        // cron 场景：env 无 __ownerScope，用 dispatch options 传入的 ownerUserId（按用户分组发送时已知）
        const logEnv = options.env.__ownerScope
          ? options.env
          : Object.create(options.env);
        if (!logEnv.__ownerScope && options.metadata && options.metadata.ownerUserId) {
          logEnv.__ownerScope = options.metadata.ownerUserId;
        }
        await writeLog(logEnv, {
          subId: options.subId,
